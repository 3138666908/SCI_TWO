"""
p1_step06_parse_mplus.py  (PartB / 阶段一 - Mplus 结果解析)

解析 research_codes/mplus/*.out，提取：
  收敛、警告数、自由参数、loglik、AIC/BIC/SaBIC、Entropy、
  VLMR-LRT p、LMR-LRT p、BLRT p、各类人数（most likely）
输出：output/p1_step06_mplus_results.csv
"""
...
def parse(path):
    ...
    # class counts (most likely membership)
...
cols = ["file", "k", "converged", "n_warn", "nfree", "loglik", "AIC", "BIC", "SaBIC",
        "entropy", "vlmr_p", "lmr_p", "blrt_p", "sizes"]
df = df[[c for c in cols if c in df.columns]]
df.to_csv(os.path.join(OUT, "p1_step06_mplus_results.csv"), index=False, encoding="utf-8-sig")
print(df.to_string())
