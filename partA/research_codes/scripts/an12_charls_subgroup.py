"""
AN-12: CHARLS subgroup forest plot data (Model 3 complete-case sample)
Exposure in forest: eCRF per 1 SD (z-scored within CHARLS eCRF-complete sample).
Within each stratum: Cox M3 (drop constant columns within stratum), HR (95% CI), events/n
  computed on the M3 complete-case sample (followup + all non-constant M3 covariates non-missing).
P-interaction: from full CHARLS M3 model with ecrf_sd * subgroup_var (numeric) interaction.
Output: an12_charls_subgroup.csv + an12_charls_subgroup_forest.png/.svg/.pdf
"""
...
def fit_within(sub, exposure):
    cols = [exposure] + [c for c in M3 if c in sub.columns]
    dfm = sub[["followup_years","mobility_event"] + cols].dropna()
    # drop constant columns within stratum
...
for gname, gvar, levels in SUBGROUPS:
    # interaction p: full CHARLS M3 with ecrf_sd*var
    inter_p = np.nan
    sub0 = d.dropna(subset=["ecrf_sd","followup_years","mobility_event", gvar] + M3, how="any")
    if gvar in ["educ_num","smk_num","drink_num","pa_num","sex_num"]:
        sub0[gvar] = pd.to_numeric(sub0[gvar], errors="coerce")
    sub0 = sub0.dropna(subset=["ecrf_sd"] + M3 + [gvar]).copy()
    sub0["inter"] = sub0["ecrf_sd"] * pd.to_numeric(sub0[gvar], errors="coerce")
    icols = ["ecrf_sd"] + M3 + [gvar, "inter"]
    icols = list(dict.fromkeys(icols))
    ...
    for val, lab in levels.items():
        ...
        cph, dfm = fit_within(sub, "ecrf_sd")
        if cph is None:
            rows.append({"subgroup": gname, "stratum": lab, "events": np.nan, "n": np.nan,
...
# interaction p only fill on non-nan rows of same subgroup
for gname in [r["subgroup"] for r in rows]:
    pass
# put interaction label only on first stratum row of each group
...
for i, r in res.iterrows():
    ...
    if g not in seen:
        seen[g] = True
        val = r["p_interaction"]
        res.at[i, "P-interaction display"] = "<0.001" if (pd.notna(val) and val < 0.001) else (f"{val:.3f}" if pd.notna(val) else "-")
    else:
        res.at[i, "P-interaction display"] = ""

res.to_csv(OUT + r"\an12_charls_subgroup.csv", index=False, encoding="utf-8-sig")
